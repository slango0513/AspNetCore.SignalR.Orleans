...
{
    internal static class OrleansLog
    {
        private static readonly Action<ILogger, string, string, Exception> _connectingToEndpoints =
            LoggerMessage.Define<string, string>(LogLevel.Information, new EventId(1, "ConnectingToEndpoints"), "Connecting to Orleans endpoints: {Endpoints}. Using Server Name: {ServerName}");
        ...
        private static readonly Action<ILogger, Guid, Exception> _publishToStream =
            LoggerMessage.Define<Guid>(LogLevel.Trace, new EventId(5, "PublishToStream"), "Publishing message to Orleans stream {Stream}.");
        ...
        public static void PublishToStream(ILogger logger, Guid streamId)
        {
            _publishToStream(logger, streamId, null);
        }
